...
// const FilterPanel = ({ filters, setFilters, categories, journals, authors }) => {
...
//               onChange={(sel) => update('category', sel && sel.length ? sel : null)}
//               compact={true}
//             />
//           </div>
...
//                 onChange={(val) => update('journal', val || null)}
...
//                 onChange={(val) => update('author', val || null)}
//                 fetchSuggestions={async (term) => {
...
//                 onChange={(e) => update('has_pdf', e.target.checked ? true : null)}
...
//               <button className="w-full text-left px-3 py-2 border rounded hover:bg-gray-50" onClick={() => {
...
//       // reset selection because new page results may be different
//       setSelectedMap({});
//     } catch (err) {
...
//     e?.preventDefault?.();
//     setFilters(prev => ({ ...prev })); // ensure chips read query if needed
...
//   // --- minimal change: remove direct performSearch call here and let effect react to state changes ---
...
//     // NOTE: previous version called performSearch(1) here via setTimeout.
//     // We removed that call and rely on the effect below to trigger the search.
//   };

//   // ---------- NEW EFFECT: re-run search when filters or query change (but only if user has searched before) ----------
//   useEffect(() => {
//     if (hasSearched) {
//       performSearch(1);
//     }
...
//           <p className="text-gray-600">Search NASA bioscience publications — enhanced with typeahead, chips & multi-select categories</p>
//         </div>
...
//             <button type="submit" disabled={loading} className="px-6 py-3 bg-blue-600 text-white rounded-lg disabled:opacity-50">
//               {loading ? 'Searching…' : 'Search'}
//             </button>

//             <button type="button" onClick={() => setShowFilters(s => !s)} className="px-4 py-2 border rounded-lg">
...
//         {showFilters && (
//           <>
//             <FilterPanel filters={filters} setFilters={setFilters} categories={categories} journals={journals} authors={authors} />
//             <div className="flex gap-3 mb-6">
//               <button onClick={() => performSearch(1)} className="px-4 py-2 bg-blue-600 text-white rounded-lg">Apply Filters</button>
//               <button onClick={() => clearFilters()} className="px-4 py-2 border rounded-lg">Clear Filters</button>
//             </div>
//           </>
//         )}
...
//               <p className="text-gray-600">Searching publications...</p>
//             </div>
//           )}
...
//                   <p className="text-gray-600">Found {totalResults.toLocaleString()} result{totalResults !== 1 ? 's' : ''}</p>
//                 </div>
//               </div>
...
//                   {results.map(pub => (
//                     <div key={pub.pmcid} className="border-l-4 border-blue-500 pl-6 py-4 hover:bg-gray-50 transition-colors rounded-r-lg">
...
//                         <span className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full ml-4 flex-shrink-0">
//                           {pub.year}
//                         </span>
//                       </div>

//                       <div className="mb-2">
//                         <Link to={`/publication/${pub.pmcid}`} className="font-semibold text-lg text-gray-900 hover:text-blue-600">
//                           {pub.title}
//                         </Link>
//                       </div>

//                       <div className="space-y-1 mb-3 text-sm text-gray-600">
//                         <p><span className="font-medium">Authors:</span> {pub.authors?.slice(0,4).join(', ')}{pub.authors?.length > 4 && ' et al.'}</p>
//                         <p><span className="font-medium">Journal:</span> {pub.journal}</p>
//                       </div>

//                       <p className="text-gray-700 leading-relaxed mb-3">{pub.abstract ? `${pub.abstract.substring(0,300)}...` : 'No abstract available.'}</p>
...
//                         </div>
//                         <Link to={`/publication/${pub.pmcid}`} className="text-sm font-medium text-blue-600 hover:text-blue-700">View Full Details →</Link>
//                       </div>
//                     </div>
//                   ))}
//                 </div>
...
//                   <div className="text-sm text-gray-700">Page {page} of {totalPages} ({totalResults} total)</div>
...
//           {!hasSearched && (
//             <div className="text-center py-12 text-gray-500">Enter a search term or open filters to begin.</div>
//           )}
//         </div>
//       </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div>
                  <div className="mb-3">
                    <label className="block text-sm font-medium text-gray-700 mb-1">Quick filters</label>
                    <div className="space-y-2">
                      <button type="button" className="w-full text-left px-3 py-2 border rounded hover:bg-gray-50" onClick={() => {
                        const top = categories.slice(0,3).map(c => c.name);
                        setFilters(prev => ({ ...prev, category: top }));
                      }}>
                        Top categories
